...
import { Image as ImageIcon, Trash2, Upload, Plus, Link2 } from "lucide-react";
import {
...
import { fetchTradesForPicker } from "../../lib/trades";

const RESULT_LABELS = { WIN: "WIN", SL: "SL (perte)", BE: "BE (breakeven)" };
...
function ScreenshotsPageInner() {
  ...
  useEffect(() => {
    ...
    fetchTradesForPicker().then(setTrades);
    return unsub;
  }, []);
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "36px 20px", display: "flex", flexDirection: "column", gap: 28 }}>
        <div className="animate-in">
          ...
          <p style={{ color: "var(--sub)", fontSize: 13, marginTop: 4 }}>
            Capture librement — lie optionnellement une paire à un trade précis pour synchroniser son résultat.
          </p>
        ...
        <form onSubmit={handleCreate} className="card card-p animate-in" style={{ display: "flex", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
          ...
          <label style={{ display: "block", flex: 1, minWidth: 180 }}>
            ...
            <input type="text" value={label} onChange={(e) => setLabel(e.target.value)} className="input" placeholder="ex: mon setup sur cette session" />
          </label>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {entries.map((entry) => (
              <EntryCard
                ...
                onSlotCleared={(slot) => updateEntryLocal(entry.id, { [slot === "avant" ? "avantUrl" : "apresUrl"]: null })}
                onSlotUploaded={(slot, url) => updateEntryLocal(entry.id, { [slot === "avant" ? "avantUrl" : "apresUrl"]: url })}
                onLinked={(tradeId) => updateEntryLocal(entry.id, { linkedTradeId: tradeId })}
                onResultChanged={(resultat, beDetail) => updateEntryLocal(entry.id, { resultat, beDetail })}
              />
            ))}
          </div>
        )}
...
function EntryCard({ entry, trades, onDeleted, onSlotCleared, onSlotUploaded, onLinked, onResultChanged }) {
  const [linking, setLinking] = useState(false);
  const [savingResult, setSavingResult] = useState(false);
  const [beDetail, setBeDetail] = useState(entry.beDetail || "");

  const linkedTrade = trades.find((t) => t.id === entry.linkedTradeId);
  ...
  async function handleLinkChange(e) {
    ...
    setLinking(true);
    try {
      await linkEntryToTrade(entry.id, tradeId);
      onLinked(tradeId);
    } finally {
      setLinking(false);
    }
  ...
  async function handleResultChange(resultat) {
    setSavingResult(true);
    try {
      await setEntryResult(entry.id, entry.linkedTradeId, resultat, beDetail);
      onResultChanged(resultat, beDetail);
    } finally {
      setSavingResult(false);
    }
  }

  async function handleBeDetailBlur() {
    if (entry.resultat !== "BE") return;
    await setEntryResult(entry.id, entry.linkedTradeId, "BE", beDetail);
    onResultChanged("BE", beDetail);
  }

  return (
    <div className="card animate-in">
      <div style={{ padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid var(--border)", flexWrap: "wrap", gap: 8 }}>
        <div>
      ...
      {/* Liaison à un trade précis + résultat synchronisé */}
      <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--border)", display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center", background: "var(--raised)" }}>
        <Link2 size={13} style={{ color: "var(--muted)", flexShrink: 0 }} />
        <select
          value={entry.linkedTradeId || ""}
          onChange={handleLinkChange}
          disabled={linking}
          className="input"
          style={{ maxWidth: 300, fontSize: 12, padding: "6px 10px" }}
        >
          <option value="">Aucun trade lié</option>
          {trades.map((t) => (
            <option key={t.id} value={t.id}>
              {t.date} · {t.paire} · {t.setup || "sans setup"}
            </option>
          ))}
        </select>
        ...
        {entry.linkedTradeId && (
          <>
            <div style={{ display: "flex", gap: 4 }}>
              {Object.entries(RESULT_LABELS).map(([key, text]) => (
                <button
                  key={key}
                  type="button"
                  disabled={savingResult}
                  onClick={() => handleResultChange(key)}
                  className="badge"
                  style={{
                    cursor: "pointer",
                    border: "1px solid " + (entry.resultat === key ? "var(--accent)" : "var(--border)"),
                    background: entry.resultat === key
                      ? (key === "WIN" ? "rgba(34,197,94,0.15)" : key === "SL" ? "rgba(239,68,68,0.15)" : "rgba(96,102,116,0.2)")
                      : "transparent",
                    color: entry.resultat === key
                      ? (key === "WIN" ? "var(--win)" : key === "SL" ? "var(--loss)" : "var(--sub)")
                      : "var(--muted)",
                  }}
                >
                  {text}
            ...
            {entry.resultat === "BE" && (
              <input
                type="text"
                value={beDetail}
                onChange={(e) => setBeDetail(e.target.value)}
                onBlur={handleBeDetailBlur}
                placeholder="ex: BE à 1R"
                className="input"
                style={{ maxWidth: 140, fontSize: 12, padding: "6px 10px" }}
              />
            )}

            <span style={{ fontSize: 10, color: "var(--muted)" }}>
              synchronisé avec {linkedTrade ? `${linkedTrade.paire} du ${linkedTrade.date}` : "ce trade"}
            </span>
          </>
        )}
      </div>
