...
/**
 * Liste légère des trades (id, date, paire, setup, result) pour le
 * sélecteur de liaison dans le module Screenshots. Un seul appel, pas
 * d'écoute continue — le module Screenshots recharge à l'ouverture.
 */
export async function fetchTradesForPicker() {
  const query = new Parse.Query(Trade);
  query.descending("date");
  query.limit(500);
  query.select("date", "paire", "setup", "result");
