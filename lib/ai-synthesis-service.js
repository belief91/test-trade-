 ...
 * Filtre uniquement les entrées de la date du jour pour éviter de mélanger
 * des communiqués de plusieurs jours dans la synthèse.
 */
async function recupererDonneesBanqueCentrale() {
  ...
  // Filtrer par date du jour (Madagascar) pour n'avoir que les événements récents
  ...
  const dateDuJourTexte = `${weekday} ${month} ${day} ${year}`;
  
  query.equalTo("dateTexte", dateDuJourTexte);
