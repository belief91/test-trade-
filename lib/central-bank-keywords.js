...
// ─────────────────────────────────────────────────────────────
// Mapping devise → banque centrale (8 institutions — NOK/SEK retirées
// le 29/08 à la demande explicite : plus suivies par le pipeline
// banque centrale)
// ─────────────────────────────────────────────────────────────
...
//
// FIX (test Fed, 20/09) : "hike", "cut", "hold", "pause", "pivot"
// retirés de politiqueMonetaire et déplacés vers TRAP_WORDS — ce sont
// des mots anglais courants à double sens (ex: "hike" = randonnée ou
// hausse de taux). Confirmé en conditions réelles : un discours Fed
// mentionnant des randonnées à Jackson Hole faisait passer 3 phrases
// hors-sujet uniquement à cause de "hikes" au sens littéral.
// ─────────────────────────────────────────────────────────────
...
// Norges Bank / Riksbank retirées le 29/08 avec DEVISE_TO_BANQUE.
// ─────────────────────────────────────────────────────────────
export const BANK_SPECIFIC_KEYWORDS = {
  ...
  // RBNZ : pas de mots-clés spécifiques listés dans le référentiel
  RBNZ: [],
};

// ─────────────────────────────────────────────────────────────
// B.9 — Mots pièges : n'entrent JAMAIS dans la liste de déclenchement.
// RÈGLE : un mot piège ne fait jamais passer une phrase à lui seul —
// la phrase ne passe que si elle contient, PAR AILLEURS, un vrai mot
// des catégories 1-8 (MONETARY_KEYWORDS ou BANK_SPECIFIC_KEYWORDS).
//
// FIX (test Fed, 20/09) : "inflation" et "growth" RETIRÉS de cette
// liste — ce sont des termes économiques centraux et unambigus dans
// un discours de banque centrale (contrairement à "hike"/"cut"/etc,
// mots anglais courants à double sens). Les garder ici les aurait
// neutralisés silencieusement puisque ce mécanisme n'était jusqu'ici
// jamais branché dans le code (voir paragraph-filter-service.js) —
// désormais branché, donc cette contradiction devait être tranchée.
// "hike", "cut", "hold", "pause", "pivot" AJOUTÉS ici, retirés de
// politiqueMonetaire — ambigus par nature (randonnée/hausse,
// couper/réduire, tenir/pause, etc.), exactement le profil visé.
// ─────────────────────────────────────────────────────────────
