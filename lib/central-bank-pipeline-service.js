...
/**
 * FIX (10/09) : exportée (était privée) pour être réutilisée par
 * recupererEvenementsBCDuJour() ci-dessous, plutôt que de dupliquer
 * cette logique de date dans module-synthesis-service.js.
 */
export function dateAujourdhuiMadagascar() {
 ...
 * ⚠️ NE PAS utiliser pour un usage quotidien (module-synthesis-service.js
 * utilise recupererEvenementsBCDuJour() ci-dessous à la place).
 */
...
/**
 * FIX (10/09) — usage quotidien : ne récupère QUE les événements "done"
 * du jour (dateTexte = aujourd'hui, format Madagascar), pour éviter que
 * module-synthesis-service.js ne retraite tout l'historique à chaque
 * appel (ce que faisait recupererTousLesEvenementsDone() par erreur).
 */
export async function recupererEvenementsBCDuJour() {
  const Pipeline = Parse.Object.extend(CLASS_NAME);
  const query = new Parse.Query(Pipeline);
