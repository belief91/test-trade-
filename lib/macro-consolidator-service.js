...
//
// FIX VOLUME (27/08) : constat sur raw/2026-08-27/calendrier-consolide.json
// réel — chaque famille liée renvoyait TOUTES les publications dans la
// fenêtre (fenetre_historique_jours), y compris plusieurs releases
// successives du MÊME indicateur (ex: "Core Inflation Rate MoM" présent
// à la fois pour le 12 août ET le 14 juillet). Pour 5 événements publiés
// le même jour, ça donnait ~1200 lignes de JSON, avec des blocs de
// contexte quasi identiques répétés d'un événement à l'autre (ex: le
// contexte "employment" est strictement le même pour core-pce, personal
// spending et gdp — recopié 3 fois). Trop de volume à interpréter pour
// la synthèse IA, risque de confusion / dilution du signal.
//
// Correction : recupererPublicationsRecentes() ne garde plus qu'UNE
// SEULE occurrence par indicateur distinct — la plus récente dans la
// fenêtre — au lieu de toutes les occurrences. Une famille avec 6
// indicateurs distincts renvoie donc 6 lignes maximum (le dernier point
// de chacun), jamais plus, peu importe la taille de la fenêtre
// historique. Le but du contexte n'est pas de donner l'historique
// complet d'un indicateur (ça, c'est le rôle de l'archive elle-même,
// pas de la synthèse du jour) mais de donner "où en est chaque
// indicateur lié en ce moment" pour que l'IA situe la publication du
// jour dans son environnement macro immédiat.
...
/**
 * FIX VOLUME (27/08) : ne garde qu'UNE occurrence par indicateur
 * distinct (normalisé sur devise+evenement, indépendamment de la date) —
 * la plus récente. Remplace l'ancien dedupliquer() qui ne dédupliquait
 * que sur (devise+evenement+date exacte), et laissait donc passer
 * plusieurs releases successives du même indicateur.
 */
