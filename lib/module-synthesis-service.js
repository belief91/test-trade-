...
//
// FIX (10/09) : chargerPromptTemplate + les fonctions construirePromptX
// et recupererDonneesX sont maintenant exportées — réutilisées par
// lib/assemblage-manuel-service.js (méthode gratuite Claude chat).
//
// FIX CRITIQUE (10/09, audit) : recupererDonneesCourbeDeTaux() lisait
// "bond-yield-curve", une clé écrite uniquement par l'ancien pipeline
// mort (genererSynthese(), plus jamais appelé depuis le branchement du
// nouveau système) — ce fichier n'était donc plus jamais régénéré.
// La vraie source active est "bond-yield-analysis" (app/api/bond-yields
// /analysis/route.js, cron quotidien 20h15 GMT+3), qui produit exactement
// la même forme {resultats: [...], devisesIgnorees: [...]}.
...
import { recupererEvenementsBCDuJour } from "./central-bank-pipeline-service";
...
export function chargerPromptTemplate(nomFichier) {
...
export async function recupererDonneesCOT() {
...
export async function recupererDonneesCourbeDeTaux() {
  const cle = genererCleDuJour("bond-yield-analysis");
  try {
    return await lireJSONDepuisR2(cle);
  } catch (err) {
 ...
 */
export async function recupererDonneesCalendrierBrut() {
...
export async function recupererDonneesGeopolitique() {
...
export async function recupererDonneesBanqueCentrale() {
  // FIX (10/09) : uniquement les événements "done" du jour, plus tout
  // l'historique — recupererTousLesEvenementsDone() est réservée à la
  // migration/rejeu (voir central-bank-pipeline-service.js).
  const resultats = await recupererEvenementsBCDuJour();
...
export function construirePromptCOT(devise, entree) {
...
export function construirePromptTaux(ligneDevise) {
...
export function construirePromptGeopo(donneesGeopo) {
...
export function filtrerContextePourDevise(contextePartage, devise) {
  const filtre = {};
...
export function construirePromptMacro(devise, dataDevise, contextePartageDevise) {
...
export function construirePromptBC(evenement) {
