...
/**
 * AJOUT (10/09) — synthèses IA de fusion journalière (narratif + biais
 * par devise, produites par les prompts modules COT/macro/BC/taux/géopo).
 * Chemin volontairement séparé de database/{module}/{date}.json pour ne
 * jamais écraser l'archive brute existante : racine "synthese/" dédiée.
 */
function genererCleSynthese(nomModule, dateStr) {
  return `synthese/${nomModule}/${dateStr}.json`;
}

function genererCleSyntheseDuJour(nomModule) {
  const maintenant = new Date();
  const offsetGMT3 = 3 * 60;
  const dateGMT3 = new Date(maintenant.getTime() + (offsetGMT3 + maintenant.getTimezoneOffset()) * 60000);
  const dateStr = dateGMT3.toISOString().split('T')[0];
  return genererCleSynthese(nomModule, dateStr);
}
