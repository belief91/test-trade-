...
//
// FIX (16/09, premier test réel) : les messages "aucune donnée ni
// secours" pour MACRO/BC étaient en prose, sans consigne de format JSON
// — Claude chat recopiait la phrase telle quelle comme contenu du bloc,
// ce qui faisait échouer JSON.parse() côté catégorisation
// (ERREUR_PARSING observée). Consigne explicite désormais : répondre
// par {} (objet JSON vide) dans ce cas, traité comme "rien à archiver"
// plutôt que comme une erreur (voir categorisation-reponse-service.js).
...
async function construireBlocCOT() {
  ...
  if (!donnees) {
    return 'AUCUNE DONNÉE COT DISPONIBLE AUJOURD\'HUI (pas de rapport CFTC ce jour — normal le week-end). Réponds uniquement par ce JSON exact : {}';
  }
...
async function construireBlocTaux() {
  ...
  if (!donnees || !Array.isArray(donnees.resultats)) {
    return "AUCUNE DONNÉE DE COURBE DE TAUX DISPONIBLE AUJOURD'HUI. Réponds uniquement par ce JSON exact : {}";
  }
...
async function construireBlocGeopo() {
  ...
  if (!donnees) {
    return "AUCUNE DONNÉE GÉOPOLITIQUE DISPONIBLE AUJOURD'HUI. Indique-le explicitement dans le bloc analytique (pas de JSON attendu ici, cf. format du prompt géopo).";
  }
...
async function construireBlocMacro(dateDuJour) {
  ...
  if (secours.statut === "NON_DISPONIBLE") {
    return "AUCUNE DONNÉE MACRO NI SYNTHÈSE DE SECOURS DISPONIBLE. Réponds uniquement par ce JSON exact : {}";
  }
...
async function construireBlocBC(dateDuJour) {
  ...
  if (secours.statut === "NON_DISPONIBLE") {
    return "AUCUN ÉVÉNEMENT BC NI SYNTHÈSE DE SECOURS DISPONIBLE. Réponds uniquement par ce JSON exact : {}";
  }
